...
dry_run=0
overwrite=0
tractometry_recon_spec="${QSIRECON_TRACTOMETRY_RECON_SPEC:-mrtrix_multishell_msmt_pyafq_tractometry}"
...
while IFS= read -r sub; do
  ...
  echo "Launching QSIRecon MRtrix/PyAFQ tractometry sub-${sub}"
...
if ! dwi_wait_all "${pids[@]}"; then
  if ((dry_run)); then
    echo "CHECK FAILED: QSIRecon MRtrix/PyAFQ tractometry dry-run inputs incomplete for one or more of ${#pids[@]} subject(s)."
  fi
  exit 1
fi

if ((dry_run)); then
  echo "CHECK PASSED: QSIRecon MRtrix/PyAFQ tractometry dry-run inputs and commands ready for ${#pids[@]} subject(s)."
fi
