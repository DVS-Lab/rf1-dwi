...
usage() {
  cat >&2 <<'USAGE'
Usage: bash run_qsirecon-dsiautotrack.sh [--sublist FILE] [--jobs N] [--dry-run] [--overwrite]
USAGE
}
...
echo "QSIRecon DSI Studio AutoTrack resource plan: up to ${max_jobs} subject job(s); each gets --nprocs ${QSIRECON_NPROCS}, --omp-nthreads ${QSIRECON_OMP_NTHREADS}, --mem ${QSIRECON_MEM_MB} MB"

args=(--workflow dsiautotrack --recon-spec dsi_studio_autotrack)
...
while IFS= read -r sub; do
  ...
  echo "Launching QSIRecon DSI Studio AutoTrack sub-${sub}"
...
if ! dwi_wait_all "${pids[@]}"; then
  if ((dry_run)); then
    echo "CHECK FAILED: QSIRecon DSI Studio AutoTrack dry-run inputs incomplete for one or more of ${#pids[@]} subject(s)."
  fi
  exit 1
fi

if ((dry_run)); then
  echo "CHECK PASSED: QSIRecon DSI Studio AutoTrack dry-run inputs and commands ready for ${#pids[@]} subject(s)."
fi
