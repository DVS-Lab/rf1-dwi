...
usage() {
  cat >&2 <<'USAGE'
Usage: bash run_qsirecon-noddi.sh [--sublist FILE] [--jobs N] [--dry-run] [--overwrite]
USAGE
}
...
echo "QSIRecon NODDI resource plan: up to ${max_jobs} subject job(s); each gets --nprocs ${QSIRECON_NPROCS}, --omp-nthreads ${QSIRECON_OMP_NTHREADS}, --mem ${QSIRECON_MEM_MB} MB"

args=(--workflow noddi --recon-spec amico_noddi)
...
while IFS= read -r sub; do
  ...
  echo "Launching QSIRecon NODDI sub-${sub}"
...
if ! dwi_wait_all "${pids[@]}"; then
  if ((dry_run)); then
    echo "CHECK FAILED: QSIRecon NODDI dry-run inputs incomplete for one or more of ${#pids[@]} subject(s)."
  fi
  exit 1
fi

if ((dry_run)); then
  echo "CHECK PASSED: QSIRecon NODDI dry-run inputs and commands ready for ${#pids[@]} subject(s)."
fi
